'''
wdSync v0.4 by Klykov Leonid
теперь копируется и эксель тоже
'''
...
def sync(filePath, serverPath):
	try:
		...
		print('Первая попытка копирования не удалась')
		time.sleep(3)
		try:
			copy(filePath, serverPath)
			print('Вторая попытка удалась')
		except IOError:
			print('Вторая попытка тоже не удалась')

class Handler(FileSystemEventHandler):
	def on_created(self, event):
		print(event)
		# if event.src_path[-3:] == 'TMP' or 'tmp':
		# 	print('tmp detected')
		# else:
	...
	def on_moved(self, event):
		print(event)
		if (event.src_path[-3:] == 'tmp' or 'TMP') and ((event.dest_path[-3:] == 'xls') or (event.dest_path[-4:] == 'xlsx')):
			sync(event.dest_path, serverPath)
